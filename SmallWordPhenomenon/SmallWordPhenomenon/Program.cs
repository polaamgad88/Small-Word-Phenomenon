...
namespace SmallWordPhenomenon
{
    ...
    class Program
    {
            ...
            Dictionary<string, Dictionary<string, Pair>> graph)
        {
            ...
            string best_node = "";
            //BEST DoS
            foreach (string node in F_Process)
            {
        ...
        {
            if (!DoS.ContainsKey(child_node_name))
                DoS.Add(child_node_name, new links_num());
            ...
            if (DoS[parent_node].value + 1 < DoS[child_node_name].value)
            {
                ...
                next_to_visit.push(new triple(DoS[child_node_name].value, RS[child_node_name].value, child_node_name));
            }
        ...
        public static void BidirectionalDijkstra(string node_a, string node_b, Dictionary<string, Dictionary<string,Pair>> graph)
        {
            ...
            //Forward path
            F_DoS.Add(node_a, new links_num());
            F_DoS[node_a].value = 0;
            F_pre_node.Add(node_a, node_a);
            F_next_to_visit.push(new triple(0, 0, node_a));
            //Backward path
            B_DoS.Add(node_b, new links_num());
            B_DoS[node_b].value = 0;
            B_pre_node.Add(node_b, node_b);
            B_next_to_visit.push(new triple(0, 0, node_b));

            while (true)
            {
                string F_parent_node = F_next_to_visit.top().third;
                string B_parent_node = B_next_to_visit.top().third;
                F_next_to_visit.pop();
                B_next_to_visit.pop();
                ...
                foreach (var (child_node_name, films) in graph[F_parent_node])
                    ReleaseNodes(F_parent_node, child_node_name, films, ref F_DoS, ref F_RS, ref F_pre_node, ref F_next_to_visit);   
                
                F_Process.Add(F_parent_node);
                if(B_Process.Contains(F_parent_node))
                {
                    //FOUND SHORTEST PATH
                    ShortestPath(node_a, F_DoS, F_RS, F_pre_node, F_Process, node_b, B_DoS, B_RS, B_pre_node, B_Process, graph);
                    return;
                }
                ...
                foreach (var (child_node_name, films) in graph[B_parent_node])
                    ReleaseNodes(B_parent_node, child_node_name, films, ref B_DoS, ref B_RS, ref B_pre_node, ref B_next_to_visit);
        ...
        public static string CreateGraph(string input_movies, string queries)
        {
